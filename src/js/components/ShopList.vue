      ...
      <div class="s-shops__scroll" ref="scrollEl">
        ...
        <ul class="s-city-list">
          <li class="s-city-list__empty"
              v-if="searchedCityList.length === 0"
          >Не найдено</li>
          <li class="s-city"
              v-for="city in searchedCityList"
              :key="city.id"
              :class="{ active: city.id === $store.state.cityId }">
            <div class="s-city__header">
              <div class="s-city__name">{{ city.name }}</div>
              <div class="s-city__note" v-if="city.id === $store.state.cityId">
              ...
              >
                ...
                <div class="s-item__bottom">
                  ...
                  <div class="s-item__contacts">
                    ...
                    <a v-if="store.vk" class="s-item__link" :href="store.vk" target="_blank">
                    ...
                    <a class="s-item__phone" v-for="phone in store.phone" :href="'tel:' + normalizePhone(phone)">{{ phone }}</a>
                  </div>
                </div>
              </li>
            </ul>
    ...
    <div class="s-shops__map" v-show="!isTabs || currentStoreId">
      ...
      <div class="s-active-shop" v-if="activeStore">
        ...
        <div class="s-active-shop__body">
          ...
          <a class="s-active-shop__phone" v-for="phone in activeStore.phone" :href="'tel:' + normalizePhone(phone)">{{ phone }}</a>
        </div>
...
export default {
  ...
  mounted() {
    ...
    if (/(storeId\d+)/i.test(document.location.hash)) {
      this.currentStoreId = parseInt(document.location.hash.match(/\d+/)[0], 10);
    }
    ...
    if (this.headerEl) {
      [].forEach.call(this.headerEl.querySelectorAll('form'), (form) => {
        form.addEventListener('submit', (event) => {
          event.preventDefault();
        });
      });

      [].forEach.call(this.headerEl.querySelectorAll('input'), (input) => {
        input.addEventListener('input', () => {
          this.searchFieldValue = input.value.trim().toLowerCase();
        });
      });
    }
  },
  computed: {
    ...
    /**
     * Перемещаем выбранный город на первое место
     * @returns {*[]}
     */
    sortedCityList() {
      const arr = [...this.$store.state.cityList];

      const index = arr.findIndex(city => city.id === this.$store.state.cityId);
      if (index >= 0) {
    ...
    /**
     * Удаляем все фейковые магазины, кроме выбранного
     * @returns {*[]}
     */
    filtredCityList() {
      return this.sortedCityList
        .filter(city => !city.isFake || city.id === this.$store.state.cityId);
    },
     ...
     */
    searchedCityList() {
      return this.filtredCityList
        .filter(item => item.name.toLowerCase().indexOf(this.searchFieldValue) !== -1);
    },
