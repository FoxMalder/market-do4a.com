...
$(() => {
      ...
      if (mutation.payload.count > 0) {
        ...
        [].forEach.call(document.querySelectorAll('[data-store-id]'), (item) => {
          ...
          if ({}.hasOwnProperty.call(mutation.payload.available_store, storeId)) {
            ...
            if (stockEl) {
              stockEl.innerHTML = global.product.textAvailable[count] + rectRatingHTML[count];
            }
  ...
  $('.p-review-form').on('submit', addReview);
  $('.p-modal-form').on('submit', addReview);
